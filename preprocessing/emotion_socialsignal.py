...
# rows = ['Wrinkled nose', 'Eyebrows Pushed Together', 'Side Eye', 'Curling Upper Lip', 'Raised Eyebrows', 'Lips Pressed Together', 'Mocking', 'Shaking Head', 'Smirk', 'Downturned lips',
...
emotion_data = pd.read_csv("../new_data/Persian/labels.csv")
contents = []
...
ss_data = pd.read_csv("../new_data/Persian/ss_persian.csv", index_col='filename')
ss_data['social_signals'] = ss_data['social_signals'].apply(eval)
...
sns.heatmap(df.loc[:, cols[0]:cols[-1]], annot=True)
plt.savefig("ss_persian_heatmapt.png", dpi = 300, bbox_inches = 'tight')